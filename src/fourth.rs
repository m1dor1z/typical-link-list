...
impl<T> Node<T> {
    ...
    #[allow(dead_code)]
    pub fn new(payload: T) -> Rc<RefCell<Self>> {
...
impl<T> List<T> {
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        self.head.take().map(|old_head| {
            ...
            Rc::try_unwrap(old_head).ok().unwrap().into_inner().payload
        })
    }

    pub fn peek_front(&self) -> Option<Ref<T>> {
        self.head.as_ref().map(|node| {
           Ref::map(node.borrow(), |node| &node.payload) 
        })
    }

    pub fn peek_front_mut(&self) -> Option<RefMut<T>> {
        self.head.as_ref().map(|node| {
            RefMut::map(node.borrow_mut(), |node| &mut node.payload)
        })
    }

    pub fn push_back(&mut self, payload: T) {
        let new_tail = Node::new(payload);
    ...
    pub fn pop_back(&mut self) -> Option<T> {
        self.tail.take().map(|old_tail| {
            ...
            Rc::try_unwrap(old_tail).ok().unwrap().into_inner().payload
        })
    }
